...
> **_NOTE:_** When configuring your search index, it's crucial to align the index's path with the embedding model you are using. For instance, if you are utilizing the Cohere model, the embeddings are typically stored under the path `embeddings`. Conversely, if you are using the OpenAI model, the embeddings might be stored under the path `embeddings_openai`. Ensure that your index configuration accurately reflects the storage path of the embeddings generated by your chosen model.
